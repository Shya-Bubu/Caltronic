...
export interface SignalPlotProps {
  ...
  samples?: number; // continuous points (default 400)
...
export default function SignalPlot({
  ...
  const discrete = useMemo(() => {
    ...
    // Map n to x evenly; treat y as sin(2π f n / Fs)
    ...
    return { pts, nMin, nMax };
  }, [amplitude, frequency, sampleRate, points, padLeft, plotW]);

  const xAxisY = yToSvg(0);

  return (
    <section className={styles.wrap} aria-label="Signal plot">
      {(title || description) ? (
        ...
        <div className={styles.control}>
          ...
          {mode === 'discrete' ? (
            <div className={styles.subnote}>
              Discrete: $x[n] = A\sin(2\pi f n / F_s)$ with $F_s = {formatNumber(sampleRate)}$ samples/s
            </div>
          ) : (
            <div className={styles.subnote}>
              Continuous: $x(t) = A\sin(2\pi f t)$ over {formatNumber(durationSeconds)} s
            </div>
          )}
        </div>
      </div>
      ...
      <div className={styles.plotShell}>
        <svg
          ...
          role="img"
          aria-label={mode === 'continuous' ? 'Continuous sine plot' : 'Discrete sine plot'}
        >
          {/* Border */}
          <rect
            x={0.5}
